...
export function CourseHeader({ course, lessonCount }: CourseHeaderProps) {
  const totalLessons =
    lessonCount ??
    course.modules.reduce((total, courseModule) => total + courseModule.lessons.length, 0);

  return (
    <div className="space-y-6">
      <div className="space-y-3">
        <p className="text-sm font-medium text-zinc-500">{course.level}</p>
        <h1 className="max-w-4xl text-4xl font-semibold tracking-normal text-zinc-950 sm:text-5xl">
          {course.title}
        </h1>
        <p className="max-w-3xl text-lg leading-8 text-zinc-600">{course.description}</p>
      </div>
