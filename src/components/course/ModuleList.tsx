...
export function ModuleList({ courseSlug, lessons, modules }: ModuleListProps) {
  const orderedModules = [...modules].sort((a, b) => a.order - b.order);

  return (
    <ol className="space-y-5">
      {orderedModules.map((courseModule) => {
        const orderedLessons = lessons.filter((lesson) => lesson.module === courseModule.title);
          ...
          <li key={courseModule.id} className="border border-zinc-200 bg-white p-6">
            <div className="space-y-4">
              <div className="space-y-2">
                ...
                <p className="text-base leading-7 text-zinc-600">{courseModule.description}</p>
                ...
                {orderedLessons.map((lesson) => (
                  <li key={lesson._id} className="py-4">
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
                      <div>
                        <p className="text-sm font-medium text-zinc-500">Lesson {lesson.order}</p>
                        ...
                        <p className="mt-1 text-sm leading-6 text-zinc-600">{lesson.description}</p>
                      </div>
